...
if args.kfold == 0:
    df, xki, xpc, xmc, xmg, ytg, pc_factor = dataset.load_datasets(args)
    args, train_data, test_data = dataset.make_datasets(args, df, xki, xpc, xmc, xmg, ytg, pc_factor)
    dict_result = dict()
    ...
    for k in range(args.kfold):
        args, train_data, test_data = dataset.make_kfold_datasets(args, kfold, k, 
                                                                  df, xki, xpc, xmc, xmg, ytg, pc_factor)
        k += 1
        if k == 4:
