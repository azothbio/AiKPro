...
parser.add_argument('--ki_n_conv1d', type=int, default=3, help='number of dense layers for kinase')
...
parser.add_argument('--ki_n_attention', type=int, default=4, help='number of multi-head attention layers for ki')
...
if args.kfold == 0:
    df, xki, xpc, xmc, xmg, x3d, ytg, pc_factor = dataset.load_datasets(args)
    args, train_data, test_data = dataset.make_datasets(args, df, xki, xpc, xmc, xmg, x3d, ytg, pc_factor)
    dict_result = dict()
    ...
    for k in range(args.kfold):
        args, train_data, test_data = dataset.make_kfold_datasets(args, kfold, k, 
                                                                  df, xki, xpc, xmc, xmg, x3d, ytg, pc_factor)
        k += 1
