...
if args.kfold == 0:
    df, xsa, xpc, xmc, xmg, ytg, pc_factor = dataset.load_datasets(args)
    args, train_data, test_data = dataset.make_datasets(args, df, xsa, xpc, xmc, xmg, ytg, pc_factor)
    dict_result = dict()
    ...
    for k in range(args.kfold):
        if k == 0:
            args, train_data, test_data = dataset.make_kfold_datasets(args, kfold, k, 
                                                                      df, xsa, xpc, xmc, xmg, ytg, pc_factor)
            k += 1
